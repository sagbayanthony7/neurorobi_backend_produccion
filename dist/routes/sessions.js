...
const normalizeDeviceTypeForPrisma = (type) => {
    return String(type || 'pulsera').toLowerCase() === 'oso' ? 'OSO' : 'PULSERA';
};
function mapSessionResponse(session) {
    ...
    return {
        ...
        sensorHistory: (session.sensorHistory || []).map((reading) => ({
            ...reading,
            deviceType: normalizeDeviceType(reading.deviceType)
        })),
        spikesLog: (session.spikesLog || []).map((spike) => ({
            ...spike,
...
router.post('/', async (req, res) => {
    try {
        ...
        const newSession = await db_1.prisma.clinicalSession.create({
            data: {
                patientId,
                specialistId,
...
router.patch('/:id/notes', async (req, res) => {
    try {
        ...
        const updated = await db_1.prisma.clinicalSession.update({
            where: { id },
            data: { notes }
        });
        res.json(updated);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        await db_1.prisma.spikesLogItem.deleteMany({ where: { sessionId: id } });
        await db_1.prisma.sensorReading.deleteMany({ where: { sessionId: id } });
        await db_1.prisma.clinicalSession.delete({ where: { id } });
